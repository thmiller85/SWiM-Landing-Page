...
export class ObjectStorageService {
  ...
  // Upload file directly to object storage for CMS images
  async uploadCMSImage(file: Buffer, originalFilename: string, mimeType: string): Promise<string> {
    const privateObjectDir = this.getPrivateObjectDir();
    if (!privateObjectDir) {
      throw new Error(
        "PRIVATE_OBJECT_DIR not set. Create a bucket in 'Object Storage' " +
    ...
    const fullPath = `${privateObjectDir}/cms-images/${filename}`;
    const { bucketName, objectName } = parseObjectPath(fullPath);
    ...
    try {
      ...
      // Set ACL policy for public access (CMS images are publicly accessible)
      ...
      return `/objects/cms-images/${filename}`;
    } catch (error) {
